<?php

namespace Application\Libraries\Core;

/**
 * Class Template - a very simple PHP class for rendering PHP templates
 */
class Template {

    /**
     * Location of expected template
     *
     * @var string
     */
    public $folder;
    private $template = 'default';
    private $title;
    private $js = [];
    private $css = [];

    /**
     * Template constructor.
     *
     * @param $folder
     */
    function __construct($folder = null) {
        if ($folder) {
            $this->set_folder($folder);
        }
    }

    /**
     * Simple method for updating the base folder where templates are located.
     *
     * @param $folder
     */
    function set_folder($folder) {
        // normalize the internal folder value by removing any final slashes
        $this->folder = $this->folder = rtrim($folder, '/');
        return $this;
    }

    function set_template($template) {
        $this->template = $template;
        return $this;
    }
    function add_css($css) {
        $this->css[] = $css;
        return $this;
    }

    function get_css() {
        return $this->css;
    }

    function add_js($js) {
        $this->js[] = $js;
        return $this;
    }

    function get_js() {
        return $this->js;
    }

    function set_title($title) {
        $this->title = $title;
        return $this;
    }

    function get_title() {
        return $this->title;
    }

    /**
     * Find and attempt to render a template with variables
     *
     * @param $suggestions
     * @param $variables
     *
     * @return string
     */
    function render($suggestions, $variables = array(), $return = false) {
        $template = $this->find_template($suggestions);

        $output = '';
        if ($template) {
            $output = $this->render_template($template, $variables);
        }
        if ($return) {
            return $output;
        }
        echo $output;
    }

    /**
     * Look for the first template suggestion
     *
     * @param $suggestions
     *
     * @return bool|string
     */
    function find_template($suggestions) {
        if (!is_array($suggestions)) {
            $suggestions = array($suggestions);
        }
        $suggestions = array_reverse($suggestions);
        $found = false;

        foreach ($suggestions as $suggestion) {
            $file = __DIR__ . "/../../{$this->folder}/{$suggestion}.php";
            if (file_exists($file)) {
                $found = $file;
                break;
            }
        }

        return $found;
    }

    /**
     * Execute the template by extracting the variables into scope, and including
     * the template file.
     *
     * @internal param $template
     * @internal param $variables
     *
     * @return string
     */
    function render_template(/* $template, $variables */) {
        ob_start();
        if (func_get_args()[1]) {
            foreach (func_get_args()[1] as $key => $value) {
                ${$key} = $value;
            }
        }

        include func_get_args()[0];
        return ob_get_clean();
    }

    function view($template, $variables = null, $return = false) {
        $content = $this->render($template, $variables, true);
        $result = $this->set_folder('Views/Template')->render($this->template, \compact('content'));
        prx($result);
        if ($return) {
            return $result;
        }
        echo $result;
    }

}
