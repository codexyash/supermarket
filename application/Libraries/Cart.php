...
class Cart {
     ...
     * @return int
     */
    public function getItemQuantity($id, $excludeHash = null) {
        $total = 0;
        if (isset($this->items[$id]) && !empty($this->items[$id])) {
            foreach ($this->items[$id] as $item) {
                if (!$excludeHash || $excludeHash != $item['hash']) {
                    $total += $item['quantity'];
                }
            }
        }
    ...
    /**
     * Get the sum of a attribute from cart without Quantity.
     *
     * @param string $attribute
     *
     * @return int
     */
    public function getExtraTotal($extra = 'price') {
        $total = 0;

        foreach ($this->items as $items) {
            foreach ($items as $item) {
                if (isset($item['extras'][$extra])) {
                    $total += $item['extras'][$extra];
                }
            }
        }
     ...
     * @return bool
     */
    public function add($id, $quantity = 1, $attributes = [], $extras = []) {
        ...
        if (isset($this->items[$id])) {
            foreach ($this->items[$id] as $index => $item) {
                if ($item['hash'] == $hash) {
                    ...
                    $this->items[$id][$index]['extras'] = $extras;
     ...
     * @return bool
     */
    public function update($id, $quantity = 1, $attributes = [], $extras = []) {
        $quantity = (preg_match('/^\d+$/', $quantity)) ? $quantity : 1;
